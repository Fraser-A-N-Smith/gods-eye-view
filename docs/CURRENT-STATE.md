...
- **Satellites**: ~880-sat core catalog (stations/visual/GPS/GLONASS/Galileo/BeiDou/GEO), tracking lands ~726km out via `viewFrom` (tracked entity owns a point graphic so the tracking camera engages), rings realign via primitive modelMatrix (no per-second rebuild flicker), optional `setParams({catalog:'dense'})` Starlink mode.
...
- Commercial and military aircraft use the same high-level FLEET model regime: 2D billboards when zoomed out, optional glTF models when closer, controlled by the DISPLAY rail `3D` toggle and `Proximity` / `All` modes. Since 2026-08-16 (Hangar fleet) models are PER-CLASS: real CC-BY GLBs for light/bizjet/turboprop/widebody/helicopter/uav (`CLASS_MODEL_REAL` in `src/data/aircraftClass.js` — meters-baked, scale 1, per-model belly/radius; provenance in `public/models/README.md`), the shared `airplane.glb` for the remaining civilian classes, and the military layer maps weight classes (real GLBs / 747 heavies / `jet.glb` fastjets, per-model heading offsets, always flat amber). 2026-08-31 added `airliner` (real A320, `a320.glb`) and `quadjet` (real A380, `a380.glb`) to `CLASS_MODEL_REAL` — sourced CC BY 4.0 from `amvlab/aircraft-models` on GitHub and re-baked to this project's convention (origin recentered, 180° nose flip, node transform folded into vertex data; the A380 asset also needed a ×0.4133 uniform rescale to reach real-world meters). `airliner` was the app's single most common class and, until this, its biggest visual-fidelity gap (a scaled-down 747 mesh). **On a separate review branch (not yet on `main`, pending human review — 2026-09-03):** `glider` and `fastjet` (both civilian and military, since `_modelSpec()` checks `CLASS_MODEL_REAL[klass]` first in both layers) also gained real assets — but original, first-party, procedurally-generated low-poly meshes (`glider.glb`, `fastjet.glb`; `scripts/build-procedural-aircraft.mjs`), not sourced third-party files: no suitable licensed source turned up for either class after a bounded search. This replaces military `fastjet`'s previous `jet.glb` fallback (a civilian "Private Jet" mesh, a known mismatch for F-16/F-22/Su-27-class fighters) with a purpose-built swept-wing fighter silhouette, and civilian `glider`'s shared-747 fallback with a long-wing, T-tail sailplane silhouette. Textured civilians carry a HEAVY tint, not a light one: `MODEL_COLOR_BLEND_AMOUNT` is `0.94` in BOTH layers under Cesium's `ColorBlendMode.MIX`, so the class colour supplies 94% of the surface and the asset's own texture ~6%. The visual direction is clean light silhouettes with only a weak diffuse contribution from the approved textures, so liveries deliberately do NOT read. IR boost raises the blend to a full `1.0`. Under NVG/FLIR (map preset or Cockpit vision) models render unlit flat-white at full alpha and scene fog is disabled (fog otherwise blacks out distant models with the globe hidden); state restores on exit.
...
Insertion is source-aware: catalog-backed missions propagate the matched satellite to the historical insertion epoch. Projected missions have no authoritative historical phase, so their orbital plane starts over the launch site and follows a launch azimuth derived from real orbital mechanics: `launchAzimuthForInclinationDeg` solves the standard sin(azimuth)=cos(inclination)/cos(latitude) relation for the named orbit's published target inclination (ISS-like 51.6° default, Sun-synchronous 97.8°, polar 90°, Molniya 63.4°, MEO 55°), picking the real-world NE (prograde) or SW (retrograde) quadrant and clamping to the site's own latitude when the target is unreachable without a plane-change dogleg — this is why, for example, a Vandenberg Sun-synchronous mission projects south while a Cape Canaveral ISS-inclination mission projects northeast, not a fixed region-keyed compass bucket. The ring itself is no longer always circular either: `approximateOrbitPath` now draws named orbits at their real published apsides, so a Geostationary Transfer Orbit renders as the visibly elongated ~250km×35,786km ellipse it actually is (previously a circle pinned at GEO altitude) and Molniya renders similarly eccentric, while a final Geostationary orbit stays a circular equatorial ring independent of the launch site's own latitude — real GEO satellites reach that plane via a later apogee-kick burn. The projected insertion advances only by the disclosed ascent duration or a ten-minute fallback, producing one continuous downrange climb into the forward orbit tangent instead of using UTC as an arbitrary phase and correcting through a 180-degree hook.
